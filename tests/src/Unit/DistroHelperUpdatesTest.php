...
class DistroHelperUpdatesTest extends UnitTestCase {
  ...
  public function testSyncActiveConfigFromSavedConfigByKeys() {
    ...
    $this->assertNotEquals($this->distroHelperUpdates->getLoggerErrors()[0],
    ...
    $bad_update = $this->distroHelperUpdates->syncActiveConfigFromSavedConfigByKeys($this->ymlOld, $this->ymlNew, ['some_nonexistent_pig#went weeeeee all the way home#distance']);
    $this->assertEquals($this->distroHelperUpdates->getLoggerErrors()[0],
      new TranslatableMarkup('Could not find a value nested at @config for either the new or old config. Is your path correct?', ['@config' => 'some_nonexistent_pig.went weeeeee all the way home.distance'])
    );
